...
### 🆕 おひっこし作法の確定（handoff安定化）

- 「おひっこし」は曖昧語ではなく、明示的な宣言とチェックリストで管理する方針を確定
- HANDOFF.md に「raw URL 単体貼り＝起動命令」という明文化ルールを追加
- 次作業 A/B/C は ROOT_INDEX / CONTEXT / PROJECT_LOG を読んだ結果のみ提示することを義務化
- ROOT_INDEX.md に「おひっこし完了チェックリスト」を追加し、成果要約の PROJECT_LOG 追記を必須化
- handoff は「編集作業場（ローカル）」と「読むだけの公開先（GitHub）」の2段構えであることを明文化
- AIは **HANDOFF.md を起点とした手順逸脱を「バグ」とみなし、即座に是正提案を行う** 運用とする
- 新チャット開始時は、AIの自由判断よりも **handoff記述の再現性を最優先** する方針を確認
...
<!-- SHIP:2026-01-20 00:54 -->
### 🆕 引っ越しパック実行（2026-01-20 00:54 JST）
- A完了：request-edit→token→view を test_request_edit_flow.ps1 で運用固定（commit 6e7bb1f）
- 結果：__RESULT__
